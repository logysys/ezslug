...
class UnifiedContentController extends Controller
{
        ...
        try {
            ...
            $formattedConversationMessages = $conversationMessages->map(function ($message) {
                ...
                return [
                    ...
                    'content_type' => $message->content_type ?? AISearchHistory::CONTENT_TYPE_AI,
    ...
    public function submitComment(Request $request)
    {
        ...
        if ($parentMessage) {
			...
			$commentData = [
					...
					'parent_id' => $parentMessage ? $parentMessage->id : null,
					'message_role' => 'user',
					'content_type' => AISearchHistory::CONTENT_TYPE_COMMENT,
	...
	{
		$validator = Validator::make($request->all(), [
			'file' => 'required|file|max:102400|mimes:jpg,jpeg,png,gif,pdf,mp4,webm,ogg,mp3,wav,html,htm', // 100MB max - Added HTML support
		...
		// Additional validation to ensure file type is allowed - UPDATED to include HTML
		$allowedMimeTypes = [
			// Images
		...
		try {
			...
			// Prepare file metadata with safe data retrieval (using variables captured before move)
			$fileMetadata = [
			...
			// For images, get dimensions if possible (using the real path from before move)
			...
			if (in_array($mimeType, ['text/html', 'application/xhtml+xml']) || $extension === 'html' || $extension === 'htm') {
				...
				$fileMetadata['sanitized'] = false; // Mark that HTML content may need sanitization when displayed
			}
			$position = null;
			if ($parentMessage) {
				$position = ($parentMessage->position ?? 0) + 1;
			...
			$uploadData = [
				...
				'thread_id' => 'thread_' . Str::random(16),
				'query' => $description ?: "Uploaded file: " . $originalName,
				'response' => null,
			...
			if ($customSlug && !$conversationId && AISearchHistory::isSlugAvailable($customSlug)) {
				$uploadData['slug'] = AISearchHistory::cleanSlugForUrl($customSlug);
			}
			...
			// Generate conversation title if this is a new conversation
			if (!$conversationId && !$parentMessage) {
				$title = $this->generateConversationTitle("File upload: " . $originalName);
				AISearchHistory::where('conversation_id', $uploadMessage->conversation_id)
					->update(['conversation_title' => $title]);
			}
			...
			// Get updated conversation
			$conversationMessages = $this->getFilteredConversationMessages($uploadMessage->conversation_id);

			return response()->json([
				'success' => true,
				'message' => 'File uploaded successfully',
				'is_html' => in_array($mimeType, ['text/html', 'application/xhtml+xml']) || $extension === 'html' || $extension === 'htm',
				'slug' => $uploadMessage->slug,
				'conversation_id' => $uploadMessage->conversation_id,
				'file' => $this->formatFileForResponse($uploadMessage),
				'conversation_messages' => $conversationMessages,
				'file_url' => asset($imageUrl),
				'access_token' => $accessToken,
				'download_url' => url('/content/file/' . $uploadMessage->slug),
			]);
	...
	private function getFilteredConversationMessages(string $conversationId)
	{
		...
		return $messages->filter(function ($message) use ($user, $sessionId) {
			if ($message->status === 'public') {
				return true;
			}
	...
	public function updateMessageContent(Request $request, $slug)
    {
        ...
        if (!in_array($message->content_type, [AISearchHistory::CONTENT_TYPE_COMMENT, AISearchHistory::CONTENT_TYPE_UPLOAD])) {
            return response()->json(['error' => 'Only comments and uploads can be edited'], 403);
        }
		...
		try {
			...
			if (!empty($shortSlug) && !empty($slugDomain)) {
				...
				if (!$existingCustomDomain) {
					...
					$domainfull = idn_to_utf8($slugDomain);
					$defaultpage = Defaultpage::whereHas('domain', function($query) use ($domainfull) {
						$query->where('domain', $domainfull);
					})->first();
					$originalFunnel = EzFunnel::findOrFail($defaultpage->handle_id);
        ...
        try {
            ...
            if (!isset($conversationId)) 
			{
                ...
                return [
                    ...
                    'slug' => $message->slug ?? null,
                    'message_role' => $message->message_role ?? null,
                    'content_type' => $message->content_type ?? AISearchHistory::CONTENT_TYPE_SOCIAL,
                    'parent_id' => $message->parent_id ?? null,
                    'query' => $message->query ?? null,
                    'response' => $message->response ?? null,
                    'file_data' => $message->file_data ?? null,
