...
const MessageEditModal: React.FC<{
    ...
    const [embedViewMode, setEmbedViewMode] = useState<'code' | 'preview' | 'split'>('code');
    ...
    const isEmbedModal = message.content_type === 'embed';

    return createPortal(
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[70] flex items-center justify-center p-4 overflow-y-auto">
            <div className={`bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-h-[90vh] overflow-y-auto ${
                isEmbedModal ? 'max-w-4xl' : 'max-w-2xl'
            }`}>
                ...
                <div className="p-6">
                    ...
                    {message.content_type === 'upload' ? (
                        ...
                        <div className="space-y-4">
                            {/* Toolbar with Subtabs and Quick Actions */}
                            <div className="flex flex-wrap items-center justify-between gap-2 p-2 bg-gray-900/60 rounded-lg border border-gray-700">
                                {/* Tab Mode Switcher */}
                                <div className="flex items-center space-x-1 bg-gray-800 p-1 rounded-lg border border-gray-700">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setEmbedViewMode('split')}
                                        className={`px-3 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 transition-colors ${
                                            embedViewMode === 'split'
                                                ? 'bg-emerald-500 text-white shadow-sm'
                                                : 'text-gray-400 hover:text-white hover:bg-gray-700/50'
                                ...
                                <div className="flex flex-wrap items-center gap-1.5">
                                    <span className="text-[11px] font-medium text-gray-400 mr-1">Insert:</span>
                                    <button
                                        type="button"
                                        onClick={() => insertSnippet('<div className="embed-row flex flex-wrap gap-4">\n  <div className="embed-card flex-1 min-w-[280px] p-4 bg-gray-800 rounded-xl border border-gray-700">\n    <!-- Item 1 -->\n  </div>\n</div>')}
                                        className="px-2 py-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded text-[11px] text-emerald-400 hover:text-emerald-300 font-medium transition-colors"
                                        title="Insert Embed Row Container"
                                    >
                                        + Row
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => insertSnippet('<div className="embed-masonry columns-1 md:columns-2 lg:columns-3 gap-4">\n  <div className="masonry-item break-inside-avoid mb-4 p-4 bg-gray-800 rounded-xl border border-gray-700">\n    <!-- Item 1 -->\n  </div>\n</div>')}
                                        className="px-2 py-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded text-[11px] text-emerald-400 hover:text-emerald-300 font-medium transition-colors"
                                        title="Insert Masonry Grid"
                                    >
                                        + Masonry
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => insertSnippet('<iframe src="https://example.com" width="100%" height="450" frameborder="0" loading="lazy" className="rounded-xl border border-gray-700 w-full"></iframe>')}
                                        className="px-2 py-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded text-[11px] text-emerald-400 hover:text-emerald-300 font-medium transition-colors"
                                        title="Insert iframe"
                                    >
                                        + iframe
                                ...
                                <div className="space-y-2">
                                    <div className="relative">
                                        <textarea
                                            ...
                                            value={content}
                                            onChange={(e) => setContent(e.target.value)}
                                            placeholder="Paste or write Embed HTML content here (e.g. <div className='embed-row'>...</div>)..."
                                            rows={14}
                                            wrap={wrapLines ? 'soft' : 'off'}
                                            disabled={isSavingState}
                                            className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl font-mono text-sm text-emerald-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 resize-y disabled:opacity-50"
                                        />
                                    </div>
                                    <div className="flex items-center justify-between text-xs text-gray-400 px-1">
                                        ...
                                        <span className="text-emerald-400/80">HTML / Embed Format</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {embedViewMode === 'preview' && (
                                <div className="space-y-2">
                                    <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 min-h-[350px]">
                                        <HtmlDocPreview 
                                            content={content || '<div class="p-6 text-center text-gray-400">No HTML content to display</div>'} 
                                            title="Embed Layout Preview" 
                                            filename="embed_preview.html" 
                                        />
                                    </div>
                                </div>
                            )}
                            ...
                            {embedViewMode === 'split' && (
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between text-xs text-gray-400 px-1">
                                            <span className="font-semibold text-emerald-400">HTML Source</span>
                                        ...
                                        <textarea
                                            ...
                                            value={content}
                                            onChange={(e) => setContent(e.target.value)}
                                            placeholder="Paste or write Embed HTML content here..."
                                            rows={14}
                                            wrap={wrapLines ? 'soft' : 'off'}
                                            disabled={isSavingState}
                                            className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl font-mono text-xs text-emerald-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 resize-y disabled:opacity-50"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between text-xs text-gray-400 px-1">
                                            <span className="font-semibold text-emerald-400">Live Preview</span>
                                        </div>
                                        <div className="bg-gray-900 border border-gray-700 rounded-xl p-3 max-h-[350px] overflow-y-auto">
                                            <HtmlDocPreview 
                                                content={content || '<div class="p-4 text-center text-gray-400">No HTML content</div>'} 
                                                title="Embed Layout Preview" 
                                                filename="embed_preview.html" 
                                            />
                                        </div>
...
const EditPanel = ({
    ...
    // ==================== HASHTAG FUNCTIONS - SMART CREATION ON SPACE / ENTER ====================
    const handleHashtagAdd = async (customInput?: string) => {
        const raw = (typeof customInput === 'string' ? customInput : hashtagInput).trim();
        if (!conversation || !raw) return;
        
        // Smartly split by whitespace or commas in case user entered/pasted multiple tokens
        const rawTokens = raw.split(/[\s,]+/).filter(Boolean);
        if (rawTokens.length === 0) return;
        
        const newTagsToAdd: string[] = [];
        const existingLower = new Set(hashtagList.map(t => t.toLowerCase()));
        
        for (const token of rawTokens) {
            // Strip leading and trailing # characters (smart handling with or without #)
        ...
        // Clear input immediately so user can smoothly type the next tag
        setHashtagInput('');
        ...
        const updatedList = [...hashtagList, ...newTagsToAdd];
        ...
        try {
            ...
            if (response.data.success) {
                setHashtagList(response.data.hashtags || updatedList);
    ...
    const handleHashtagRemove = async (tagToRemove: string) => {
        ...
        const updatedList = hashtagList.filter(t => t !== tagToRemove);
    ...
    // Helper function to get hashtag display
                            ...
                            <div className="bg-gray-800/50 rounded-2xl border border-gray-700/50 p-5 transition-all duration-200 hover:border-gray-600">
                                <div className="flex items-start gap-6">
                                    <div className="flex-1 space-y-4">
                                        ...
                                        {/* Hashtags Field - SMART SPACE / ENTER ADDITION WITHOUT ADD BUTTON */}
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1.5">
                                                <span className="flex items-center gap-2">
                                            ...
                                            {/* Smart Hashtag input without add button */}
                                            <div className="relative">
                                                <div className="flex items-center border-2 border-gray-600 rounded-xl bg-gray-900/50 transition-all duration-200 focus-within:border-yellow-400 hover:border-gray-500 px-3">
                                                    <span className="text-sm text-gray-500 font-mono select-none mr-1">#</span>
                                                    <input
                                                        type="text"
                                                        value={hashtagInput}
                                                        onChange={(e) => {
                                                            const value = e.target.value;
                                                            // If user enters a space or comma (via typing or paste), smartly extract & create hashtags
                                                            if (value.includes(' ') || value.includes(',')) {
                                                                handleHashtagAdd(value);
                                                            } else {
                                                        ...
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter' || e.key === ' ' || e.key === ',') {
                                                                if (hashtagInput.trim()) {
                                                                    e.preventDefault();
                                                                    handleHashtagAdd(hashtagInput);
                                                                } else if (e.key === ' ') {
                                                                    // Don't insert leading spaces
                                                                    e.preventDefault();
                                                                }
                                                            }
                                                        }}
                                                        ...
                                                        className="flex-1 py-2.5 outline-none text-sm font-medium bg-transparent text-white placeholder-gray-500"
                                                        data-tooltip-id="main-tooltip"
                                            ...
                                            {hashtagList.length > 0 ? (
                                                <div className="mt-3 flex flex-wrap gap-2">
                                                    {hashtagList.map((tag) => (
                                                        <span
                                                            key={tag}
                                                            className="group inline-flex items-center gap-1.5 bg-yellow-400/10 text-yellow-400 px-3 py-1.5 rounded-full border border-yellow-400/30 text-sm font-medium transition-all hover:bg-yellow-400/20"
                                                        >
                                                ...
                                                <p className="mt-2 text-xs text-gray-500">No hashtags added yet. Type above and press Space or Enter to add.</p>
                                            )}
...
export default function AIHistory({
    ...
    useEffect(() => {
        ...
        if (searchQuery.trim()) {
            ...
            filtered = filtered.filter((conv) => {
                // 1. Title, query, response preview, user email, status, model, conversation ID
                if (
                            ...
                            msg.id === message.id 
                                ? { 
                                    ...
                                    ...(message.content_type === 'ai' 
                                        ? { response: content }
                                        : (message.content_type === 'comment' || message.content_type === 'embed')
                                        ? { query: content }
    ...
    const handleUpdateMessageStatus = async (message: ConversationMessage) => {
        try {
            ...
            if (response.data.success) {
                ...
                const errMsg = response.data.message || response.data.error || 'Failed to update message status';
                showAlert(errMsg, 'error');
                throw new Error(errMsg);
            }
        } catch (error: any) {
            console.error('Failed to update message status:', error);
            const errorMessage = error.response?.data?.message || error.response?.data?.error || error.message || 'Failed to update message status';
            showAlert(errorMessage, 'error');
            throw error;
        }
    };
    ...
    const handleDeleteMessage = async (message: ConversationMessage) => {
        try {
            ...
            if (response.data.success) {
                ...
                const errMsg = response.data.message || response.data.error || 'Failed to delete message';
                showAlert(errMsg, 'error');
                throw new Error(errMsg);
            }
        } catch (error: any) {
            console.error('Failed to delete message:', error);
            const errorMessage = error.response?.data?.message || error.response?.data?.error || error.message || 'Failed to delete message';
            showAlert(errorMessage, 'error');
            throw error;
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to delete conversation. Please try again.';
            showAlert(errorMessage, 'error');
        } finally {
