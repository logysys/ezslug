...
export default function AISearchView({ 
    ...
    useEffect(() => {
        if (prevMessageCount > 0 && conversation.length > prevMessageCount) {
            scrollToBottom();
        }
        setPrevMessageCount(conversation.length);
    }, [conversation]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 100 * 1024 * 1024) {
            const errorMsg = `❌ File size exceeds 100MB limit. Current size: ${formatFileSize(file.size)}. Maximum allowed: 100MB.`;
            setUploadError(errorMsg);
            showAlert(errorMsg, 'error');
            if (fileInputRef.current) {
                fileInputRef.current.value = '';
    ...
    // ============================================================
    const handleCommentSubmit = useCallback(async (parentSlug?: string) => {
        if (!canInteract) {
            setShowLoginPrompt(true);
            return;
        }
        ...
        try {
            // Use the provided parentSlug or default to search.slug
            const slugToUse = parentSlug || search.slug;
            
            const response = await axios.post('/content/comment', {
                content: commentContent,
                conversation_id: search.conversation_id,
                parent_slug: slugToUse,
            }, {
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
                },
            ...
            if (response.data.success) {
                ...
                showAlert(parentSlug ? 'Reply posted successfully!' : 'Comment posted successfully!', 'success');
            } else {
    ...
    }, [canInteract, commentContent, search.conversation_id, search.slug, isOwner, isPrivateAccessGranted]);
        ...
        const getMarkdownContent = () => {
            ...
            if (isHtml) {
                return `<div style="width: 100%;">
                    <iframe src="${fileData.url}" style="width: 100%; height: 500px; border: 1px solid #e5e7eb; border-radius: 0.5rem; display: block;" title="${fileData.original_name}" sandbox="allow-same-origin allow-scripts allow-popups allow-forms allow-modals">
        ...
        if (isHtml) {
            return (
                <div className="w-full break-words" style={{ width: '100%' }}>
                    <div 
                        dangerouslySetInnerHTML={{ __html: getMarkdownContent() }} 
                        style={{ width: '100%' }}
                    />
                </div>
            );
    ...
    const renderComment = (content: string) => {
        const isFullHtmlDocument = content.trim().startsWith('<!DOCTYPE') || 
        ...
        return (
            <div className="break-words place-content-center justify-center items-center">
                {isFullHtmlDocument ? (
                    <div className="comment-html-preview">
                        <iframe
                            srcDoc={content}
                            title="HTML Preview"
    ...
    const renderSocialPost = (message: ConversationMessage) => {
        ...
        const hasFullHtmlDoc = content.trim().startsWith('<!DOCTYPE') || 
                              content.trim().startsWith('<html') ||
                              (content.includes('<head>') && content.includes('<body>'));
        ...
        return (
            <div className="break-words">
                ...
                {hasFullHtmlDoc ? (
                    <div className="social-post-html-preview rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
    ...
    const renderMessage = (message: ConversationMessage) => {
        ...
        const parentMessage = isReply ? conversation.find(m => m.id === message.parent_id) : null;
            ...
            >
                {isReply && parentMessage && (
                    <div className="text-xs text-gray-400 mb-1 flex items-center gap-2">
                        ...
                        <span>
                            Replying to {parentMessage.user?.name || 'Guest'}
                            {parentMessage.content_type === 'comment' && ' (Comment)'}
                            {parentMessage.content_type === 'social' && ' (Social)'}
                            {parentMessage.content_type === 'upload' && ' (Upload)'}
                            {parentMessage.content_type === 'ai' && ' (AI)'}
                        </span>
                    </div>
                )}
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <MessageReactionsBar
                            ...
                            onAddComment={async (text) => {
                                // Set comment content and call handleCommentSubmit with the message slug for replies
                                setCommentContent(text);
                                setActiveTab('social');
                                setContentSubTab('embed');
                                // Pass the message slug as parent_slug for proper reply threading
        ...
        return (
            <div className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-400">• Images, PDF, Video, Audio, HTML files up to 100MB</span>
                    <button
                        onClick={() => {
                ...
                {!selectedFile ? (
                    <div 
                        ...
                        data-tooltip-id="main-tooltip"
                        data-tooltip-content="Click to upload or drag & drop"
                    >
        ...
        <>
            ...
            <div className="min-h-screen bg-[#FCFCFC] text-slate-800">
                            ...
                            {!isBannerVisible && (
                                <div className="flex justify-end mb-4">
                                    <button 
                                        ...
                                        className="text-xs font-bold text-gray-400 hover:text-[#22c55e] flex items-center gap-2 bg-gray-100 px-3 py-1.5 rounded-full transition-all border border-gray-200 shadow-sm"
                                    >
                ...
                <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-[60]">
                    <button 
                        onClick={scrollToTop}
                        className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center border border-blue-600 shadow-md hover:shadow-lg transition-all text-white hover:bg-blue-600"
                        data-tooltip-id="main-tooltip"
                        data-tooltip-content={tooltips?.ai_search_view_scroll_top || "Scroll to top"}
                    >
                        <FontAwesomeIcon icon={faChevronUp} />
                    </button>
                    <button 
                        onClick={scrollToBottom}
                        className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center border border-green-600 shadow-md hover:shadow-lg transition-all text-white hover:bg-green-600"
                        data-tooltip-id="main-tooltip"
                        data-tooltip-content={tooltips?.ai_search_view_scroll_bottom || "Scroll to bottom"}
                    >
