                        ...
                        {parseHashtags(message.hashtag).length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
                                {parseHashtags(message.hashtag).map((tag) => (
                                    <a
                                        key={tag}
                                        href={`/search/hashtag?searchhashtag=${encodeURIComponent(tag)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                                    ...
                                                    {parseHashtags(related.hashtag).length > 0 && (
                                                        <div className="flex flex-wrap gap-1 mt-1.5">
                                                            {parseHashtags(related.hashtag).slice(0, 3).map((tag) => (
                                                                <span key={tag} className="text-[10px] font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-200">
                                                                    #{tag}
                                                                </span>
                                                            ))}
                                                        </div>
